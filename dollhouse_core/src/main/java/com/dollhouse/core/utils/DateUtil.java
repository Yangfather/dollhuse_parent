...
public class DateUtil {
	...
	public static Long convert(String date) {
		...
		try {
			if(date.length() > 10) 
				...
				return (dateShortFormat.get().parse(date).getTime()/1000);		
			}	
		} catch (ParseException e) {
			return null;
		}
	 ...
	 * @return
	 */
	public static Long getYearFirstDay(Date date) {
	    Calendar calendar = Calendar.getInstance();
	    calendar.setTime(date);
	...
	public static boolean checkShortDate(String dateStr) {
		try {
			dateShortFormat.get().parse(dateStr);
		} catch (Exception e) {
			return false;
		}
		return true;
	...
	public static Long shortTimeFormat(Long shortDate) {
		try {
			return convert(dateShortFormat.get().parse(shortDate.toString()));
		} catch (Exception e) {
			return shortDate;
		}
	}
