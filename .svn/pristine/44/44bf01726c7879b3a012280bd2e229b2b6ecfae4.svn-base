...
<script type="text/javascript" src="dollhouse_user/assets/sockjs/sockjs.js"></script>
</head>
<body>
...
<script>
    ...
    var wsServer = null;
    var ws = null;
    wsServer = "ws://${pageContext.request.getServerName()}:${pageContext.request.getServerPort()}/dollhouse_user/chatServer";
			...
			<div class="am-u-lg-12" style="height:400px;border:1px solid #999;overflow-y:scroll;">
				<ul id="chatContent" class="am-comments-list am-comments-list-flip">
					<li id="msgtmp" class="am-comment" style="display:none;">
						<a href="">
					        <img class="am-comment-avatar" style="width: 50px;height: 50px;border-radius:50px;" src="dollhouse_user/assets/images/chat/other.jpg" alt="用户"/>
					    </a>
	...
	<script type="text/javascript">
		$(function(){
			...
			var socket = new WebSocket("ws://${pageContext.request.getServerName()}:${pageContext.request.getServerPort()}/dollhouse_user/chatServer");
			socket.send(JSON.stringify({
	            message : {
	                content : message,
	                /* from : '${userid}', */
	                from:'客服',
	                to : "顾客1",      //接收人,如果没有则置空,如果有多个接收人则用,分隔
	                time : getDateFull()
	            },
	            type : "message"
	        }));
