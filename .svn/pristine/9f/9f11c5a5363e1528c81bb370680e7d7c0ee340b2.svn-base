...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<%=path %>/assets/css/login.css">
    <link rel="stylesheet" type="text/css" href="<%=path %>/assets/css/datepicker/css/jquery-ui.css">
    <link rel="stylesheet" type="text/css" href="<%=path %>/assets/js/widgets/layer/skin/layer.css">
...
<body>
    <div class="login">
				...
				<div class="login-cont">
					...
					<div class="login-item">
							...
							<input type="text" class="login-input" name="verifycode" id="verifycode"
								...
								src="<%=path %>/common/public/verifycode1/1"
								onclick="this.src='<%=path %>/common/public/verifycode1/'+Math.random();" />
					</div>
					<div class="login-item">
    	...
    	<div class="copyright">
              Copyright 玩具屋婴幼儿玩具商城  ©2015-2018
	    </div>
    </div>
    	...
    	$("#loginForm").on("submit",function(){
    		if($('#userName').val()=="" || $('#password').val()==""){
	    		...
	    		$.ajax({
	    			...
	    			$("#loading").hide();
	    			 if(data.status == 200) {
		                    window.location.href = "<%=path %>/common/returnIndex";
		                } else {
