...
<body class="bodybg">
	...
	<div class="page">
		...
		<input type="button" value="修改" class="sch-btn" onclick="sbt()">
	</div>
</div>
<script type="text/javascript">
	function sbt(){
		...
		/* alert(lowerPrice) */
		var heigherPrice=document.getElementById("heigherPrice").value;
		/* alert(heigherPrice) */
		var status=document.getElementById("status").value;
		/* alert(status); */
		var type=document.getElementsByName("type");
		var price=document.getElementsByName("price");
		var imgUrl=document.getElementsByName("imgUrl");
		var types=[];
    	var prices=[];
    	...
    	/* alert(types+";"+prices); */
    	var urls="";
    	for(var i = 0, j = imgUrl.length; i < j; i++){
    		var img=imgUrl[i].src;
    		img=img.replace("http://","");
    		img=img.replace("localhost:8080/","");
    		img=img.replace("dollhouse_admin/","");
    		img=img.replace("dollhouse_user/","")
    	...
    	/* alert(urls); */
    	$.ajax({
    		url:"<%=path%>/admin/manager/change",
    		type:"post",
    		data:{"nameId":goodsId,"name":name,"color":color,"lowerPrice":lowerPrice,"heigherPrice":heigherPrice,
    			"status":status,"types":types,"prices":prices,"urls":urls},
    		dataType:"json",
